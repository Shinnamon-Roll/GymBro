...
\begin{lstlisting}[language=JavaScript, caption={ซอร์สโค้ดจากไฟล์ server.js}, label={code:server-js-autodelete}]
    ...
    try {
        ...
        if (expired.length > 0) {
            for (const user of expired) {
                ...
                // Delete user
                await user.destroy();
                logAction("Auto Delete Expired", `Deleted user ID: ${user.id}...`);
            }
            // Reorder
...
\begin{enumerate}
    ...
    \item ดำเนินการวนลูปเพื่อลบข้อมูล:
    \begin{enumerate}
        \item ลบข้อมูล \texttt{TrainingSessions} ที่เกี่ยวข้องกับลูกค้ารายนั้นก่อน (เพื่อป้องกันข้อผิดพลาด Foreign Key Constraint)
        \item ลบข้อมูล \texttt{Customers}
    \end{enumerate}
...
const sequelize = new Sequelize(
  ...
  process.env.DB_PASSWORD,
  {
    ...
    dialectOptions: {
      ssl: {
        require: true,
        rejectUnauthorized: false, // รองรับ Self-signed certificate
      },
    },
    logging: false, // ปิด SQL logging เพื่อความสะอาดของ Console
  }
);
\end{lstlisting}
...
const Customers = sequelize.define("customers", {
  ...
  memberType: { type: DataTypes.STRING, defaultValue: "Standard" }, // Standard, Premium, VIP
  memberLevel: { type: DataTypes.STRING, defaultValue: "Basic" }, // Basic, Silver, Gold
  memberStartDate: { type: DataTypes.DATEONLY, defaultValue: DataTypes.NOW },
  memberEndDate: { type: DataTypes.DATEONLY },
});
\end{lstlisting}
...
const TrainingSessions = sequelize.define("training_sessions", {
  // ... fields definition
});

// กำหนดความสัมพันธ์ (Associations)
TrainingSessions.belongsTo(Customers, { foreignKey: "customerId", as: "customer" });
TrainingSessions.belongsTo(Trainers, { foreignKey: "trainerId", as: "trainer" });
TrainingSessions.belongsTo(GymEquipments, { foreignKey: "equipmentId", as: "equipment" });
\end{lstlisting}
